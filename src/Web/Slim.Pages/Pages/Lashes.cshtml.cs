...
namespace Slim.Pages.Pages
{
    public class LashesModel : PageModel
    {
        ...
        public LashesModel(ILogger<LashesModel> logger, ICacheService cacheService, IPageSection<PageSection> pageSectionsBaseStore, IBaseStore<RazorPage> razorPagesBaseStore)
        {
            _logger = logger;
            ...
            _pageId = razorPages
                         .FirstOrDefault(x =>
                             string.Compare(x.PageName, "Lashes", StringComparison.OrdinalIgnoreCase) == 0)?.Id ??
                     0;
        }
